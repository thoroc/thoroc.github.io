...
import { buildKnowledgeMessage } from './surface-knowledge'
...
describe('buildKnowledgeMessage', () => {
  ...
  it('returns undefined for empty or unparseable input', () => {
    expect(buildKnowledgeMessage([], 'test-kb')).toBeUndefined()
    expect(buildKnowledgeMessage(['not json'], 'test-kb')).toBeUndefined()
  })
  ...
  it('falls back to unknown domain when missing', () => {
    const message = buildKnowledgeMessage([JSON.stringify({})], 'test-kb')
