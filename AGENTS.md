...
`.context/` holds the durable session record. Create and index files with the `context-file` and `context-index` skills:

- Types: `plan`, `finding`, `analysis`, `follow-up`, `learning`, `handover` — each maps to a matching subdirectory (`plans/`, `findings/`, `analysis/`, `follow-ups/`, `learnings/`, `handover/`).
- Filenames are date-first (`YYYY-MM-DD-<slug>.md`), enforced by `check-context-filenames.sh`.
- `.context/index.yaml` is generated by `regenerate-context-index.sh` — run it after creating or updating context files so the index reflects new entries.
- Learnings document rules from past sessions; handovers capture session hand-off state. Both use the standard frontmatter (`title`, `type`, `status`, `date`).
